...
with pd.ExcelWriter(output_file, engine="xlsxwriter") as writer:
    ...
    for area in sorted(df["area_nm_completo"].dropna().unique()):
        ...
        risk_worksheet.write(risk_row, 0, area)
        risk_worksheet.write(risk_row, 1, area)
        risk_row += 1
        
        risk_worksheet.write(risk_row, 0, "Risco")
        risk_worksheet.write(risk_row, 1, "Quantidade")
        risk_row += 1
        ...
        for _, row in risk_counts.iterrows():
            ...
            risk_worksheet.write(risk_row, 0, row["Risco"])
            risk_worksheet.write(risk_row, 1, row["Quantidade"])
            risk_row += 1
