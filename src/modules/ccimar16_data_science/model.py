...
class CCIMAR16Model(QObject):
    ...
    def adjust_table_structure(self):
        """Verifica e cria a tabela 'ccimar16_db' se não existir."""
        query = QSqlQuery(self.db)
        if not query.exec("SELECT name FROM sqlite_master WHERE type='table' AND name='ccimar16_db'"):
            print("Erro ao verificar existência da tabela:", query.lastError().text())
        if not query.next():
            print("Tabela 'ccimar16_db' não existe. Criando tabela...")
            self.create_table_if_not_exists()
        else:
            pass
            # print("Tabela 'ccimar16_db' existe. Verificando estrutura da coluna...")

    def create_table_if_not_exists(self):
        """Cria a tabela 'ccimar16_db' com a estrutura definida, caso ainda não exista."""
        query = QSqlQuery(self.db)
        if not query.exec("""
            CREATE TABLE IF NOT EXISTS ccimar16_db (
                status TEXT, dias TEXT, prorrogavel TEXT, custeio TEXT                               
            )
        """):
            print("Falha ao criar a tabela 'ccimar16_db':", query.lastError().text())
        else:
            print("Tabela 'ccimar16_db' criada com sucesso.")
    ...
    def insert_or_update_data(self, data):
        ...
        try:
            ...
            if "no such table" in str(e):
                QMessageBox.warning(None, "Erro", "A tabela 'ccimar16_db' não existe. Por favor, crie a tabela primeiro.")
                return
            else:
