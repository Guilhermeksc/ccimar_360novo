...
import json
from .treeview import load_config
...
class EditDialog(QDialog):
    ...
    def excluir_objeto(self):
        resposta = QMessageBox.question(
            self,
            "Confirmar Exclusão",
            "Deseja realmente excluir este objeto?",
        ...
        if resposta == QMessageBox.StandardButton.Yes:
            try:
                ...
                if config:
                    ...
                    nr_to_delete = self.objeto_auditavel.get("nr")
                    # Filtra a lista removendo o objeto com o "nr" correspondente
                    novos_objetos = [obj for obj in objetos if obj.get("nr") != nr_to_delete]
                    if len(novos_objetos) == len(objetos):
                        QMessageBox.critical(self, "Erro", "Objeto não encontrado para exclusão.")
                        return
                    config["objetos_auditaveis"] = novos_objetos
                    with open(CADASTRO_OBJETOS_AUDITAVEIS_PATH, "w", encoding="utf-8") as f:
                        json.dump(config, f, indent=4, ensure_ascii=False)
                    QMessageBox.information(self, "Sucesso", "Objeto excluído com sucesso!")
                    self.deleted = True
                    self.accept()  # Fecha o diálogo após a exclusão
            except Exception as e:
                QMessageBox.critical(self, "Erro", f"Falha ao excluir objeto: {e}")
