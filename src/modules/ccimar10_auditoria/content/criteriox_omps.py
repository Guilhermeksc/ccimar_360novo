...
def create_x(title_text, database_model):
    ...
    print(f"🔍 DEBUG: database_model is of type {type(database_model)}")  # Debugging
    
    if not hasattr(database_model, "insert_organizacao_militar"):
        raise TypeError(f"❌ ERROR: Expected `database_model` to be an instance of `CCIMAR11Model`, but got {type(database_model)} instead!")
